...
private void muter(double m){
	  for(int i=0; i< individus.size(); i++)
	 ((Individu) individus.get(i)).setFitness(((Individu) individus.get(i)).getfitness()+((Individu) individus.get(i)).getfitness()*m);
  }
  ...
  private Population reproduire(){
	 ...
	 for(int i =0; i<taille20prcent;i++){
		 Individu i2 =((Individu) individus.get(i)).clone();
		 nouvpop.add(i2);
	 }
	 /** reproduction, croiser et muter les 80% individus*/
	 for(int j=0; j<individus.size()-taille20prcent; j++){
          Individu i1, i2, i3;		 
		 i1 = (Individu) individus.get(new Random().nextInt(taille20prcent));		
		 i2 =(Individu) individus.get(new Random().nextInt(taille20prcent));
		 i3=i1.croiser(i2);
		 i3.muter(); 
		 nouvpop.add(i3);
	 }
