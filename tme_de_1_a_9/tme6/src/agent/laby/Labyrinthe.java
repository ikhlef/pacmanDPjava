...
//		return true;
//	}
	/**Point estentoureDeMurs renvoie point si le point n'est pas un mur*/
	public Point estEntoureDeMurs() {
		int imax = Xsize()-1;
		int jmax = Ysize()-1;
		for (int i=0;i< Xsize();i++){
			if (carte[i][jmax]!=ContenuCase.MUR) return new Point (i, jmax);
			if (carte[i][0]!=ContenuCase.MUR) return new Point (i, 0);
		}
		for (int j=0;j<Ysize();j++){
			if (carte[0][j]!=ContenuCase.MUR) return new Point (0, j);
			if (carte[imax][j]!=ContenuCase.MUR) return new Point (imax, j);
		}
		return null;
	}
	/**
